    ...
    if len(texto_final.strip()) < 20:
        try:
            ...
            for pagina in doc:
                ...
                # DPI reduzido para 150 (antes era 300). Isso evita o erro de "not enough memory".
                try:
...
def extrair_dados_pdf(arquivo):
    """
    Extrai dados aplicando Regex ajustado para o layout da GUIA 12 2025.
    """
    dados = {
    ...
    try:
        ...
        # 2. DATA (Manual conforme solicitado)
        dados["DATA ATEND."] = "" 
    ...
    if st.button("💾 Salvar Fatura e Gerar Arquivo"):
        if df_dados.empty: 
            ...
            try:
                ...
                pasta_destino = os.path.join("data", f"{mes_nome}_{ano_competencia}")
                ...
                with open(caminho_completo, "rb") as f: 
                    st.download_button("📥 Baixar Word", f, file_name=nome_arquivo)
            ...
            if col_btn2.button("📄 Regenerar Documento Word"):
                try:
                    ...
                    with open(nome_arq, "rb") as f: 
                        st.download_button("📥 Baixar", f, file_name=nome_arq)
                        
                except Exception as e: st.error(f"Erro: {e}")
    else: 
        st.warning("Nenhuma fatura encontrada.")
